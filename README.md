...
Railway uses the `npm start` script from `package.json`. Vercel uses `vercel.json`.

Required Railway variables: `MONGO_URI`, `JWT_SECRET`, `CLIENT_URL`, `CLIENT_URLS`, `MIN_WITHDRAWAL_AMOUNT`, `REFERRAL_LEVEL_1_BONUS`, `REFERRAL_LEVEL_2_BONUS`, `REFERRAL_LEVEL_1_RATE`, `REFERRAL_LEVEL_2_RATE`, `TASK_COMPLETION_PERCENT`, `MIN_WATCH_SECONDS`, `DEPOSIT_WALLET_ADDRESS`, `DEPOSIT_WALLET_TRC20`, `DEPOSIT_WALLET_BEP20`, `DEPOSIT_WALLET_ERC20`, `EMAIL_USER`, `EMAIL_PASS`.
